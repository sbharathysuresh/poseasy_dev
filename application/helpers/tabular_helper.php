...
/*
Get the header for the assets tabular view
*/
function get_assets_manage_table_headers()
{
	$CI =& get_instance();
	...
	$headers = array(
		...
		array('purchase_date' => $CI->lang->line('common_purchase_date')),
		array('purchase_value' => $CI->lang->line('common_purchase_value')),
...
/*
Get the header for the picklist tabular view
*/
function get_picklist_manage_table_headers()
{
	$CI =& get_instance();

	$headers = array(
		
		array('item' => $CI->lang->line('common_item')),
		array('purchase' => $CI->lang->line('common_purchase')),
		array('quantity' => $CI->lang->line('common_quantity')),
		array('supplier' => $CI->lang->line('common_supplier')),
		array('email_pick_no' => $CI->lang->line('common_pick_no')),
		array('item_delivery_expectation' => $CI->lang->line('common_item_delivery_expectation')),
	);
...
/*
Get the html data row for the asset
*/
function get_asset_data_row($asset)
{
	$CI =& get_instance();
	...
	return array (		
		...
		'purchase_date' => to_datetime(strtotime($asset->purchase_date)),
		'location' => $asset->location,		
		'messages' => empty($person->phone_number) ? '' : anchor("Messages/view/$asset->id", '<span class="glyphicon glyphicon-phone"></span>',
			array('class'=>'modal-dlg', 'data-btn-submit' => $CI->lang->line('common_submit'), 'title'=>$CI->lang->line('messages_sms_send'))),
		'edit' => anchor($controller_name."/view/$asset->id", '<span class="glyphicon glyphicon-edit"></span>',
			array('class'=>'modal-dlg', 'data-btn-submit' => $CI->lang->line('common_submit'), 'title'=>$CI->lang->line($controller_name.'_update'))
	));
}

/*
Get the html data row for the picklist
*/
function get_picklist_data_row($picklist)
{
	$CI =& get_instance();

	$controller_name = strtolower(get_class($CI));
	$item = $CI->Item->get_info($picklist->item);
	$supplier = $CI->Supplier->get_info($picklist->supplier);
	return array (		
		'id' => $picklist->id,
		'item' => $item->name,
		'purchase' => $picklist->purchase,
		'item_delivery_expectation' => to_datetime(strtotime($picklist->item_delivery_expectation)),
		'supplier' => $supplier->company_name,	
		'quantity' => $picklist->quantity,
		'pick_no' => $picklist->pick_no,		
		'messages' => empty($person->phone_number) ? '' : anchor("Messages/view/$picklist->id", '<span class="glyphicon glyphicon-phone"></span>',
			array('class'=>'modal-dlg', 'data-btn-submit' => $CI->lang->line('common_submit'), 'title'=>$CI->lang->line('messages_sms_send'))),
		'edit' => anchor($controller_name."/view/$picklist->id", '<span class="glyphicon glyphicon-edit"></span>',
			array('class'=>'modal-dlg', 'data-btn-submit' => $CI->lang->line('common_submit'), 'title'=>$CI->lang->line($controller_name.'_update'))
	));
}
