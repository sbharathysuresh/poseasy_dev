...
class Asset extends CI_Model
{
	...
	*/
	public function get_info($asset_id)
	{
		...
		else
		{
			...
			foreach($this->db->list_fields('assets') as $field)
			{
				$assetObj->$field = '';
			}
	...
	*/
	...
	/*
	Gets information about multiple assets
	*/
	...
	*/
	public function save_asset(&$assets, $asset_id = FALSE)
	{
		$success = FALSE;
		...
		//if(parent::save($assets, $asset_id))
		//{
			if(!$asset_id || !$this->exists($asset_id))
			{
			//	$customer_data['person_id'] = $person_data['person_id'];
				$success = $this->db->insert('assets', $assets);
			}
			else
			{
				$this->db->where('assets.id', $asset_id);
				$success = $this->db->update('assets', $assets);
			}
		//}
	...
	*/
	public function search($search, $rows = 0, $limit_from = 0, $sort = 'name', $order = 'asc', $count_only = FALSE)
	{
		...
		if($count_only == TRUE)
		{
			$this->db->select('COUNT(assets.id) as count');
		}
		...
		$this->db->group_start();
			...
			$this->db->or_like('purchase_value', $search);			;
		$this->db->group_end();
		$this->db->where('deleted', 0);
	...
	*/
	public function delete_list($asset_ids)
	{
		$success = FALSE;
		...
		$this->db->trans_start();
			$this->db->where_in('id', $asset_ids);
			$success = $this->db->update('assets', array('deleted'=>1));
		$this->db->trans_complete();
