...
class Picklists extends Persons
{
	private $_list_id;

	public function __construct()
	{
		parent::__construct('picklists');
	...
	{
		$data['table_headers'] = $this->xss_clean(get_picklist_manage_table_headers());

		$this->load->view('picklist/manage', $data);
	}
	...
	*/
	public function get_row($row_id)
	{
		...
		$data_row = $this->xss_clean(get_picklist_data_row($person, $stats));
	...
	*/
	public function search()
	{
		...
		$picklist = $this->Picklist->search($search, $limit, $offset, $sort, $order);
		$total_rows = $this->Picklist->get_found_rows($search);
		...
		//	$stats = $this->Customer->get_stats($asset->id);
			...
			$data_rows[] = $this->xss_clean(get_picklist_data_row($pl));
		}
	...
	*/
	public function view( $picklist_id = -1)
	{
		...
		//$suppliers = $this->Supplier->get_info(-1);
		$picklist = $this->Picklist->get_info($picklist_id);
		$data['item_id'] = $this->xss_clean($picklist->item);
		$data['quantity'] = $this->xss_clean($picklist->quantity);		
		$data['purchase'] = $this->xss_clean($picklist->purchase);
		$data['person_id'] = $this->xss_clean($picklist->supplier);
        $data['pick_no'] = $this->xss_clean($picklist->pick_no);
		$data['picklist_id'] = $this->xss_clean($picklist->id);
		$data['item_delivery_expectation'] = $this->xss_clean($picklist->item_delivery_expectation);
		//$data['suppliers'] = $this->xss_clean($suppliers->company_name);
		//$data['suppliers'] = $this->xss_clean($picklist->id);
		...
		$data['selected_supplier'] = $picklist->supplier;		

		$itemsDD = array('' => $this->lang->line('items_none'));
		foreach($this->Item->get_all()->result_array() as $row)
		{
			$itemsDD[$this->xss_clean($row['item_id'])] = $this->xss_clean($row['name']);
		}
		$data['itemsDD'] = $itemsDD;
		$data['selected_item'] = $picklist->item;
		...
		$this->load->view("picklist/form", $data);
	}

	/*
	Inserts/updates a customer
	*/
	public function save($picklist_id = -1)
	{
		$item = $this->xss_clean($this->input->post('item'));
		$quantity = $this->xss_clean($this->input->post('quantity'));
		$supplier = $this->xss_clean($this->input->post('supplier'));
        $purchase = $this->xss_clean($this->input->post('purchase'));
        $pick_no = $this->xss_clean($this->input->post('pick_no'));
	...
	//	$purchase_date = date_create_from_format($this->config->item('dateformat') . ' ' . $this->config->item('timeformat'), $this->input->post('purchase_date'));
		$date_formatter = date_create_from_format($this->config->item('dateformat') . ' ' . $this->config->item('timeformat'), $this->input->post('item_delivery_expectation'));
	//	$date_formatter = date_create_from_format($this->config->item('date_or_time_format'), $this->input->post('purchase_date'));
		...
		$picklist = array(
			...
			'item' => $item,
			'quantity' => $quantity,
			'item_delivery_expectation' => $date_formatter->format('Y-m-d H:i:s'),
		...
		if($this->Picklist->save_picklist($picklist, $picklist_id)){
			if($picklist_id == -1)
			{
				echo json_encode(array('success' => TRUE,
								'message' => $this->lang->line('picklist_successful_adding') . ' ' . $item . ' ' . $supplier,
								'id' => $this->xss_clean($picklist_id)));
			}
			else // Existing customer
			{
				echo json_encode(array('success' => TRUE,
								'message' => $this->lang->line('picklist_successful_updating') . ' ' . $item . ' ' . $supplier,
								'id' => $picklist_id));
			}
		}else // Failure
		{
			echo json_encode(array('success' => FALSE,
							'message' => $this->lang->line('picklist_error_adding_updating') . ' ' . $item . ' ' . $supplier,
							'id' => -1));
		}
	...
	*/
	...
	{
		$picklist_to_delete = $this->input->post('ids');
		//echo "Assets to Delete: ".$picklist_to_delete;
		$picklist_info = $this->Picklist->delete_list($picklist_to_delete);

		if($this->Picklist->delete_list($picklist_to_delete))
		{
			echo json_encode(array('success' => TRUE, 'message' => $this->lang->line('picklist_one_or_multiple') . ' ' . count($picklist_to_delete) . ' ' . $this->lang->line('picklist_one_or_multiple'), 'ids' => $picklist_to_delete));
		}
		else
		{
			echo json_encode(array('success' => FALSE, 'message' => $this->lang->line('picklist_cannot_be_deleted'), 'ids' => $picklist_to_delete));
		}
	...
	*/
	...
	{
		$this->load->view('picklist/form_csv_import', NULL);
	}
	...
	public function do_csv_import()
	{
		...
		{
			echo json_encode(array('success' => FALSE, 'message' => $this->lang->line('picklist_csv_import_failed')));
		}
		else
		{
			if(($handle = fopen($_FILES['file_path']['tmp_name'], 'r')) !== FALSE)
			{
				...
				{
					$message = $this->lang->line('picklist_csv_import_partially_failed') . ' (' . count($failCodes) . '): ' . implode(', ', $failCodes);

					echo json_encode(array('success' => FALSE, 'message' => $message));
				}
				else
				{
					echo json_encode(array('success' => TRUE, 'message' => $this->lang->line('picklist_csv_import_success')));
				}
			}
			else
			{
				echo json_encode(array('success' => FALSE, 'message' => $this->lang->line('picklist_csv_import_nodata_wrongformat')));
			}
		}
	}
}
